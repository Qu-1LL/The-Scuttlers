...
public readonly record struct WorldDebugInspection(Creature? Creature, string Tooltip)
{
    public bool HasValue => Creature is not null;
}

public static class WorldDebugInspector
{
    public static WorldDebugInspection Inspect(Cave cave, WorldPoint point, bool showHitboxes)
    {
        if (showHitboxes && FindNearestCreature(cave, point) is { } creature)
        {
            return new WorldDebugInspection(creature, FormatCreature(creature));
        }
