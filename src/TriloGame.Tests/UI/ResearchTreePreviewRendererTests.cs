...
public sealed class ResearchTreePreviewRendererTests
{
    [Fact]
    public void CalculatePreviewLayout_FitsTheWholeCuratedTreeInsideThePreviewBounds()
    {
        var featureTree = Assert.IsType<FeatureTree>(TriloDex.Global.FindFeatureTree("B2"));
        var bounds = new Rectangle(20, 30, 240, 150);

        var layout = ResearchTreePreviewRenderer.CalculatePreviewLayout(
    ...
    [Fact]
    public void CalculatePreviewLayout_SupportsDraftBranchesThroughTheSamePreviewPath()
    {
    ...
    {
        Assert.Equal(0.55f, ResearchTreePreviewRenderer.ClampZoom(0.1f));
        Assert.Equal(1.2f, ResearchTreePreviewRenderer.ClampZoom(1.2f));
        Assert.Equal(2.25f, ResearchTreePreviewRenderer.ClampZoom(9f));
    }
}
